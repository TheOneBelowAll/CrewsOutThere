...
func parentRegexSetup() {
	// Captures parent-child relationship
	regexString := "(?i)^I am (not )?responsible for (\\d{10,11})[\\s]*$"

	// Compile regex (Will crash if something went wrong)
	parentRegex = regexp.MustCompile(regexString)
}
...
func flyNotifRegexSetup() {
	...
	// Captures the specified airport(s)
	regexString2 := "([ ,]*)([a-zA-Z]+)"

	// Compile regex (Will crash if something went wrong)
	flyNotifRegexHelper = regexp.MustCompile(regexString2)
}
...
func roleRegexSetup() {
	...
	// Captures the specified role(s)
	regexString2 := "([ ,]*)([a-zA-Z]+)"

	// Compile regex (Will crash if something went wrong)
	roleRegexHelper = regexp.MustCompile(regexString2)
}
