...
func InitConf() {
	...
	//rawData, err := os.ReadFile("./../encryption/cot.conf")
	rawData, err := os.ReadFile("/etc/cot.conf")
	if err != nil {
		log.Fatal(err)
	}
	data := string(rawData)
...
func GetReporterContact() string {
	return config["isCotDownReceiver"]
}

func GetOwnerContact() string {
	return config["Aran"]
}
